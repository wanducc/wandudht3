...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:123456@goto_host:3306/wandudb'
...
def create_user(name,password,email):
    if name is None:
        name = input('输入用户名(默认admin):') or 'admin'
    if password is None:
...
def changepassword(newpassword):
    ...
    if newpassword is None:
        newpassword = generate_password_hash(getpass('新密码:'))
