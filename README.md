...
#### 万度是从浩瀚的DHT网络（UDP）中获取磁力链接（magnet）信息的搜索引擎，主要分为爬虫、网站、数据库、索引，爬虫基于socket、bencode库，网站基于flask库，数据库为mysql，索引为sphinx，请勿用于非法用途
只是一个copy。示例：http://wandu.cli  已经无法访问了。
使用方法：
确定号你要建立的目录：
```Bash
cd /root 
...
我们后续的例子都已 /root/wandudht/ 这个目录为例了
...
A：在/etc/nginx/nginx.conf文件内修改，多个域名用空格隔开，修改完成后执行nginx -s reload生效
...
A：修改simdht_worker.py里的Getspeed=后面的数字，越大爬取越快，越小爬取越慢
...
A：执行 mysqldump -uroot -p wandudb>/root/wandudb.sql  导出数据库        //将提示输入当前密码，数据库导出后存在/root/wandudb.sql

**Q：数据库备份后，现在重新安装了程序，如何导入旧数据？**

A：执行 mysql -uroot -p wandudb</root/wandudb.sql       //假设你的旧数据库文件是/root/wandudb.sql，将提示输入当前密码，输入后耐心等待
